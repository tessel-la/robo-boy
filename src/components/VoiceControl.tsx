...
import './VoiceControl.css'; // Create later
...
const VoiceControl: React.FC<VoiceControlProps> = ({ ros }) => {
  ...
  useEffect(() => {
    ...
    console.log(`Advertised ${VOICE_CMD_TOPIC}`);

    return () => {
      voiceCmdTopic.current?.unadvertise();
      console.log(`Unadvertised ${VOICE_CMD_TOPIC}`);
