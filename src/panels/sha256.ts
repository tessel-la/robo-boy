...
export const getSha256Integrity = async (input: Uint8Array): Promise<string> => {
  if (globalThis.crypto?.subtle) {
    try {
      ...
      // Some embedded webviews expose SubtleCrypto but reject digest calls.
    }
  }
  return `sha256-${bytesToBase64(sha256(input))}`;
};
