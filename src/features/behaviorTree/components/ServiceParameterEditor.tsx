  ...
  if (subfields?.length) {
    return subfields
      ...
      .map(sf => {
        ...
        if (typeof sv === 'number') return `${sf.name}:${isFloatType(sf.rosType) ? sv.toFixed(1) : sv}`;
        return `${sf.name}:${sv}`;
      })
      .join('  ');
  }
...
type NavFrame = { kind: 'list'; path: string[] } | { kind: 'edit'; path: string[]; field: ActionFieldSchema };
...
const ComplexEditView: React.FC<{
  ...
  const typeLabel = field.arrayLen >= 0 ? `${field.rosType}[${field.arrayLen || '…'}]` : field.rosType;

  return (
    <div className="ape-edit-view">
      <span className="ape-type-badge" style={{ alignSelf: 'flex-start' }}>
        {typeLabel}
      </span>
      <textarea
...
const ServiceParameterEditor: React.FC<ServiceParameterEditorProps> = ({ nodeData, ros, onSave, onClose }) => {
  ...
  useEffect(() => {
    ...
    const initialVals: Record<string, any> = hasExisting ? existing : ((template as Record<string, any>) ?? {});
  ...
  const handleDragStart = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const onMove = (ev: PointerEvent) => setPanelHeight(Math.max(180, startH + (startY - ev.clientY)));
    const onUp = () => {
  ...
  const panelStyle: React.CSSProperties = panelHeight ? { height: `min(${panelHeight}px, calc(100% - 24px))` } : {};

  return (
    <div className="ape-overlay" onClick={onClose}>
      <div className="ape-panel" ref={panelRef} style={panelStyle} onClick={e => e.stopPropagation()}>
        {/* Drag handle */}
        ...
        {!canGoBack && <div className="ape-subtitle">{nodeData.serviceType || 'Unknown type'}</div>}
        ...
        <div className="ape-body">
          {viewMode === 'json' ? (
            ...
            <FieldEditView
              ...
              onChange={val => setValueAtPath([...currentFrame.path, currentFrame.field.name], val)}
            />
          ) : null}
        </div>
        ...
        <div className="ape-footer">
          ...
          <button className="ape-btn ape-btn-save" onClick={handleSave} disabled={isLoading} type="button">
            Save
          </button>
        </div>
      </div>
