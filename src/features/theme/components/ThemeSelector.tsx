...
const PaletteIcon = () => (
  ...
  >
    ...
    <circle cx="12" cy="12.5" r=".5" fill="currentColor" />
  </svg>
);
const StarIcon = () => (
  <svg
...
const ThemeSelector: React.FC<ThemeSelectorProps> = ({
  ...
  return (
    <div className="theme-selector-container">
      ...
      >
        <span className="icon-wrapper">{getIcon(currentTheme?.iconId || currentThemeId)}</span>
        <span className="theme-selector-copy">
          <span>Theme</span>
          <span>{currentTheme?.name || 'Choose theme'}</span>
        </span>
        <svg className={`theme-selector-chevron ${isMenuOpen ? 'is-open' : ''}`} viewBox="0 0 16 16" aria-hidden="true">
          <path d="m4 6 4 4 4-4" />
        </svg>
      </button>

      {isMenuOpen && (
        <div className="theme-popup-menu" id={themeListId} role="group" aria-label="Themes">
          {themes.map(themeData => (
            <div key={themeData.id} className="theme-item-row">
              <button
                type="button"
              ...
              >
                {getIcon(themeData.iconId || themeData.id)}
